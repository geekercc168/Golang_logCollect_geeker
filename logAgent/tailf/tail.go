...
func createNewTask(conf CollectConf) {
	// 初始化Tailf实例
	//TailFile filename 字段只能使用绝对路径
	global.Log.Info("configs.LogPath:%s", conf.LogPath)
	...
	if errTail != nil {
		global.Log.Error("收集文件[%s]错误: %v", conf.LogPath, errTail)
		return
	}
...
func InitTail(conf []CollectConf, chanSize int) (err error) {
	...
	// 加载配置项
	if len(conf) == 0 {
		global.Log.Error("无效的日志collect配置: ", conf)
	}

	// 循环导入
	for _, v := range conf {
		//logs.Info("开始createNewTask " + fmt.Sprintf("%+v", v))
		createNewTask(v)
	}
...
func readFromTail(tailObj *TailObj) {
	for true {
		...
		case <-tailObj.exitChan:
			global.Log.Warn("tail obj 退出, 配置项为conf:%v", tailObj.conf)
			return
		}
	}
}
